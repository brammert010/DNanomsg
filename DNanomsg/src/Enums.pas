...
type
  ...
  TSocketOptionTcp =
    (
      NoDelay = NN_TCP_NODELAY
    );

  TSocketOptionSurvey =
    (
      SurveyorDeadline = NN_SURVEYOR_DEADLINE
    );

  TSocketOptionRequest =
    (
        RequestResendInterval = NN_REQ_RESEND_IVL
    );

  TSocketOptionSub =
    (
        Subscribe = NN_SUB_SUBSCRIBE,
        Unsubscribe = NN_SUB_UNSUBSCRIBE
    );
